...
namespace Gurux.Net
{
    ...
    Thread m_ReceiverThread;
        ...
        public void Open()
        {
            Log.Information("Open {Host}:{Port}", HostName, port);
            Close();
            try
            {
                syncBase.Open();
                ...
                {
                    Log.Information("Host{Host}Port:{Port} communicationProtocol == NetworkType.Tcp", HostName, port);
                    if (!isVirtual)
                    {
                        Log.Information("Host{Host}Port:{Port} !isVirtual", HostName, port);
                        socket = new Socket(family, SocketType.Stream, ProtocolType.Tcp);
                    }
                    if (!isServer)
                    {
                        Log.Information("Host{Host}Port:{Port} IPEndPoint:{ep}, family:{Family}", HostName, port, ep, family);
                        ClientConnect(ep);
                    }
                }
